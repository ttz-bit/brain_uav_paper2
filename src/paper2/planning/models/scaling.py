...
class FixedObsScaler(nn.Module):
    """Fixed physical scaling for the Paper1 SNN-TD3 observation vector."""

    def __init__(self, scenario: ScenarioConfig, state_dim: int, clip_value: float = 5.0) -> None:
        super().__init__()
        nearest_zone_count = int(scenario.nearest_zone_count)
        expected_state_dim = 5 + 3 + 4 + 4 * nearest_zone_count
        if state_dim != expected_state_dim:
            raise ValueError(
                f"FixedObsScaler expected state_dim={expected_state_dim} for "
                f"nearest_zone_count={nearest_zone_count}, got {state_dim}."
            )
