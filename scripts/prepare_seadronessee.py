...
def parse_args():
    ...
    parser.add_argument(
        "--raw-root",
        type=str,
        default=r"D:\datasets\SeaDronesSee",
        help="SeaDronesSee raw root",
    )
    parser.add_argument(
        "--out-root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Square crop size",
    )
    parser.add_argument(
        "--max-sequences",
        type=int,
        default=None,
        help="Only process first N sequences per split for preview",
    )
    parser.add_argument(
        "--qc-per-split",
        type=int,
        default=30,
        help="How many QC overlays / crops to save per split",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite crops / manifests if exists",
    )
...
def process_split(
    ...
    if split == "test":
        ann_dir = raw_root / "sot" / "test_annotations_first_frame"
    else:
        ann_dir = raw_root / "sot" / f"{split}_annotations"
    ...
    for seq_id in tqdm(seq_ids, desc=f"Processing {split}"):
        ...
        if len(seq_items) != len(ann_lines):
            ...
            print(
                f"[WARN] Length mismatch in split={split}, seq={seq_id}: "
                f"{len(seq_items)} frames vs {len(ann_lines)} annotations. "
                f"Using first {min(len(seq_items), len(ann_lines))} aligned pairs."
            )
        ...
        for i in range(pair_count):
            ...
            record = {
                ...
                "sequence_id": str(seq_id),
                "frame_id": frame_id,
                "source_image_path": str(img_path).replace("\\", "/"),
                ...
                "gsd": None,
                "meta": {
                    "sot_local_frame_name": local_frame_name,
