...
def _target_from_sample(sample) -> np.ndarray:
    ...
    bw = float(sample.bbox_xywh[2]) / max(1.0, float(w))
    bh = float(sample.bbox_xywh[3]) / max(1.0, float(h))
    return np.array([bw, bh], dtype=np.float32)
...
def main() -> None:
    ...
    for rank, i in enumerate(vis_indices):
        ...
        h, w_img = s.image.shape[:2]
        gt_wh = _target_from_sample(s)
        pred_w = int(np.clip(pred[0], 0.0, 1.0) * w_img)
        pred_h = int(np.clip(pred[1], 0.0, 1.0) * h)
        gt_w = int(np.clip(gt_wh[0], 0.0, 1.0) * w_img)
        gt_h = int(np.clip(gt_wh[1], 0.0, 1.0) * h)
        cx, cy = w_img // 2, h // 2

        vis = s.image.copy()
        cv2.rectangle(vis, (cx - gt_w // 2, cy - gt_h // 2), (cx + gt_w // 2, cy + gt_h // 2), (0, 255, 0), 2)
        cv2.rectangle(
            vis,
            (cx - pred_w // 2, cy - pred_h // 2),
            (cx + pred_w // 2, cy + pred_h // 2),
            (0, 0, 255),
            2,
        )
        cv2.putText(
            vis,
            f"gt_wh=({gt_w},{gt_h}) pred_wh=({pred_w},{pred_h})",
            (8, 18),
