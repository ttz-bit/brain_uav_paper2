...
def main() -> None:
    ...
    x_train = np.stack([_to_tensor_image(train_ds[i].image) for i in range(len(train_ds))], axis=0).astype(np.float32)
    y_train = np.stack([_target_from_sample(train_ds[i]) for i in range(len(train_ds))], axis=0).astype(np.float32)
    x_val = np.stack([_to_tensor_image(val_ds[i].image) for i in range(len(val_ds))], axis=0).astype(np.float32)
    y_val = np.stack([_target_from_sample(val_ds[i]) for i in range(len(val_ds))], axis=0).astype(np.float32)
    ...
    for ep in range(epochs):
        ...
        print(f"[SNN-FIT] epoch={ep+1:03d}/{epochs} train_loss={train_loss:.6f} val_loss={val_loss:.6f}")
