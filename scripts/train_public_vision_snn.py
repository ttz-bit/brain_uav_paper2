...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num-steps", type=int, default=12, help="SNN simulation steps")
    parser.add_argument("--beta", type=float, default=0.95, help="LIF membrane decay")
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "outputs" / "train" / "public_vision_snn"),
    )
...
def _import_torch_and_snn():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required for train_public_vision_snn.py. "
    ...
    try:
        ...
        raise RuntimeError(
            "snntorch is required for train_public_vision_snn.py. "
            "Please install with: pip install snntorch"
        ) from e
...
def main() -> None:
    ...
    class _SmallSNN(nn.Module):
        ...
        def _rate_encode(self, x):
            # Bernoulli rate coding over time.
        ...
        def forward(self, x):
            ...
            # Use spike-rate as normalized prediction in [0,1].
            return torch.stack(spk_out_list, dim=0).mean(dim=0)

    model = _SmallSNN(beta=float(args.beta), num_steps=int(args.num_steps)).to(device)
    ...
    for epoch in range(epochs):
        ...
        print(f"[SNN] epoch={epoch + 1:03d}/{epochs}, loss={epoch_loss:.6f}")
