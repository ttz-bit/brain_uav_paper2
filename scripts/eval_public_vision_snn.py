...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weights-path", type=str, required=True, help="Path to model.pth from train_public_vision_snn.py")
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "outputs" / "eval" / "public_vision_snn"),
    )
...
def _import_torch_and_snn():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required for eval_public_vision_snn.py. "
    ...
    try:
        ...
        raise RuntimeError(
            "snntorch is required for eval_public_vision_snn.py. "
...
def main() -> None:
    args = parse_args()
    torch, nn, snn, surrogate = _import_torch_and_snn()
